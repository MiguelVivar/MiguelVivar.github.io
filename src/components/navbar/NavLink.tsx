    ...
    >
      ...
      >
        ...
        <AnimatePresence>
          {isHovered && !isActive && (
            <motion.div 
              className="absolute inset-0 bg-emerald-300/5 rounded-lg z-0"
        ...
        <motion.div 
          className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-emerald-300 to-transparent rounded-full`}
        ...
        {isActive && (
          <motion.div 
            className="absolute -top-1 left-3 right-3 h-px bg-emerald-300/30"
        ...
        <AnimatePresence>
          {isActive && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.3 }}
              className="absolute inset-0 rounded-lg bg-emerald-300/5 -z-10"
            />
          )}
        </AnimatePresence>
      </motion.div>

      <span className={`transition-colors duration-300 flex items-center relative z-10 ${
        isActive 
          ? 'text-emerald-300' 
